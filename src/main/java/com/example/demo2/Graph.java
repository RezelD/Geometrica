...
public class Graph extends AnchorPane {
    ...
    private RezPoint selectedPoint;
    private RezPoint panOffset = new RezPoint(0,0);
    private final RezPoint lastMousePos = new RezPoint(0,0);

    private final ArrayList<RezPoint> points = new ArrayList<>();
    private final ArrayList<RezPoint> distance = new ArrayList<>();
    private final ArrayList<RezPoint> intersections = new ArrayList<>();

    Formula<DistanceFormulaInput, Double> pointDistance = new DistanceFormula();

    private RezPolygon selectedPolygon;

    private final ArrayList<RezPolygon> polygons = new ArrayList<>();

    private boolean draggingPoint = false;
    private boolean movingPolygon = false;
    ...
    public void setTextUpdater(TextUpdater textUpdater) {

        this.textUpdater = textUpdater;
    }

    public RezPolygon getSelectedPolygon() {

        return selectedPolygon;
    }

    public ArrayList<RezPoint> getMeasuredPoints() {
    ...
    void press(MouseEvent event) throws NonInvertibleTransformException {
        ...
        if (selectedPolygon != null) {
            for (RezPoint point : selectedPolygon.getPolygon()) {
                ...
                if (pointDistance.compute(new DistanceFormulaInput(point, mouseWorld)) <= grabDistance) {
        ...
        for (RezPolygon polygon : polygons) {
            for (RezPoint point : polygon.getPolygon()) {
                ...
                double dist = pointDistance.compute(new DistanceFormulaInput(point, mouseWorld));
    ...
    void move(MouseEvent event) throws NonInvertibleTransformException {
        ...
        if (event.getButton() == MouseButton.PRIMARY) {
            ...
            for (RezPoint intersection : intersections) {
                double distance = pointDistance.compute(new DistanceFormulaInput(intersection, mouseWorldLocation));
            ...
            if (!draggingPoint && selectedPoint == null && selectedPolygon != null && !distance.isEmpty()) {
                int count = 0;
                for (RezPoint ignored : selectedPolygon.getPolygon()) {

                    RezPoint moved = selectedPolygon.getPolygon().get(count);
                    RezPoint offset = distance.get(count);

                    moved.set(mouseWorldLocation.getX() + offset.getX(), mouseWorldLocation.getY() + offset.getY());
